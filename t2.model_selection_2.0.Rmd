...
```{r}
#file paths
splitOne_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t2_BCT_vol_splitONE_2.0/"
splitTwo_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t2_BCT_vol_splitTWO_2.0/"
...
```{r}
save_fits = TRUE
all_fits_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/t2_BCT_fits.csv"
if(save_fits == TRUE){
...
#distribution of convergence_warn_ends among phenotypes
table(sOne_fits %>% filter(convergence_warn_end == 1) %>% select(phenotype))

#distribution of convergence_warn_ends among model terms
table(sOne_fits %>% filter(convergence_warn_end == 1) %>% select(mu_formula))
table(sOne_fits %>% filter(convergence_warn_end == 1) %>% select(sigma_formula))
table(sOne_fits %>% filter(convergence_warn_end == 1) %>% select(nu_formula))
...
#distribution of convergence_warn_ends among model terms
table(sTwo_fits %>% filter(convergence_warn_end == 1) %>% select(mu_formula))
table(sTwo_fits %>% filter(convergence_warn_end == 1) %>% select(sigma_formula))
table(sTwo_fits %>% filter(convergence_warn_end == 1) %>% select(nu_formula))
```
...
```{r}
sum(grepl("age:sex", sOne_best$mu_formula)) #1
sOne_best[(grep("age:sex", sOne_best$mu_formula)), 'label']

sum(grepl("age:sex", sTwo_best$mu_formula)) #0
#sTwo_best[(grep("age:sex", sTwo_best$mu_formula)), 'label']
```
