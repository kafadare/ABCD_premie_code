...
centilest2_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/Centile_CSVs/regional_t2_volume_centiles.csv"
centilest1_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/Centile_CSVs/regional_baseline_volume_centiles.csv"
```
...
centiles_t1 <- read.csv(centilest1_filename)  %>% select(-1)
centiles_t2 <- read.csv(centilest2_filename)  %>% select(-1)
#take difference of centiles at t1 and t2
centiles_all <- merge(centiles_t1, centiles_t2, by = "src_subject_id")
diff_cols <- grep(".t2_centiles$", names(centiles_all), value = TRUE)
for (col in diff_cols) {
  t1_col <- sub(".t2", "", col)
  if (t1_col %in% names(centiles_all)) {
    centiles_all[[paste0(t1_col, "_diff")]] <- (qnorm(centiles_all[[col]]) - qnorm(centiles_all[[t1_col]]))/qnorm(centiles_all[[t1_col]])
  }
}
rm(centiles_t1,centiles_t2)
...
#Merge dataframes
test <- merge(full_test_df, full_abcd[,c("scrn_birthcomp","birth_weight_lbs", "neonatal_comp_total","race_ethnicity", "src_subject_id")], by = c("src_subject_id"), all.x = TRUE)
...
```
##save male and female birthweight and GA files to convert to birthweight percentiles via the Fenton excel sheet calculator.
```{r}
#male_bw <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% filter(sex == "M")
#female_bw <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% filter(sex == "F")
# 
#write.csv(male_bw, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_new2.csv")
#write.csv(female_bw, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/female_birthweight_new2.csv")

## Download above files and convert birthweight to birthweight percentiles by gestational age
filename_bwcentile_boy <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_new.csv"
filename_bwcentile_girl <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/female_birthweight_new.csv"
all_birthweight <- rbind(read.csv(filename_bwcentile_boy, na.strings=c("","NA")), read.csv(filename_bwcentile_girl, na.strings=c("","NA"))) %>% select(-1)
#all_birthweight$bw_category <- ifelse(is.na(all_birthweight$birth_weight_grams), NA, all_birthweight$bw_category)
test_prs <- merge(test_prs, all_birthweight %>% select(c("src_subject_id", "birth_weight_grams", "bw_category", "bweight_zscore", "bweight_percentile")), by = c("src_subject_id"), all.x = TRUE)

```
rsync -avz /home/kafadare/tsclient/ekafadar/Documents/Grad_School/BGDLab/ABCD_data/male_birthweight_new.csv /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/

##Inter-variable correlations
```{r}
#bw pgs
cor.test(abcd_prs$bw_pgs, abcd_prs$bw)
cor.test(test_prs$bw_pgs, test_prs$birth_weight_grams)
#bw pgs with bw percentile
cor.test(test_prs$bw_pgs, test_prs$bweight_percentile)
#ga pgs
...
cor.test(test_prs$gestAge, test_prs$bweight_percentile)
#bw and birth comp
...
cor_matrix <- cor(test_prs[,c("gestAge", "bw", "neonatal_comp_total", "ga_pgs", "bw_pgs", "bweight_percentile", "General_p")], use = "pairwise.complete.obs")
...
reg_table_full <- as.data.frame(t(sapply(full_test_df[, grepl("centiles_diff", names(full_test_df))], function(x) {
  x_zscore <- qnorm(x)
...
```{r}
reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles_diff", "", rownames(reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
bw_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
```{r}
bwp_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
...
```{r}
bwc_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  bwc_var <- (test_prs$bw_category)
  x_zscore <- qnorm(x)
  #also omit those cells in bwc_var to match the two vectors
  bwc_var[is.infinite(x_zscore)] <- NA
  bwc_var[is.na(x_zscore)] <- NA
  x_zscore[is.infinite(bwc_var)] <- NA
  bwc_var[is.infinite(bwc_var)] <- NA
  x_zscore[is.na(bwc_var)] <- NA
  bwc_var <- na.omit(bwc_var)
  #omit cells with infinite values after applying qnorm
  x_zscore[is.infinite(x_zscore)] <- NA
  x_zscore <- na.omit(x_zscore)
...
####BWcategory
```{r}
sga_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles", "", rownames(sga_reg_table_full_sig_MC)), name_mapping$abbreviation)]

lga_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles", "", rownames(lga_reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
bwc_reg_table_full <- as.data.frame(t(sapply(df[, grepl("centiles_diff", names(df))], function(x) {
  bwc_var <- (df$bw_category)
  x_zscore <- qnorm(x)
  #also omit those cells in bwc_var to match the two vectors
  bwc_var[is.infinite(x_zscore)] <- NA
  bwc_var[is.na(x_zscore)] <- NA
...
neo_comp_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
bw_pgs_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
ga_pgs_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
```{r}
ga_pgs_reg_table_full_sig$label <- name_mapping$region_name[match(sub("_centiles", "", rownames(ga_pgs_reg_table_full_sig)), name_mapping$abbreviation)]
...
gp_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
bw_zscore <- z_score(test_prs$birth_weight_lbs)
neo_comp_zscore <- z_score(test_prs$neonatal_comp_total)
reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
```{r}
#regression
bwp_ga_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
  ga_zscore <- z_score(test_prs$gestAge)
  bwp_zscore <- qnorm(test_prs$bweight_percentile)
  x_zscore <- qnorm(x)
...
bw_wh_reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles_diff", names(test_prs))], function(x) {
...
```{r}
galga_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles", "", rownames(galga_reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
####interaction effect (surface plots)
```{r}
gabwp_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles", "", rownames(gabwp_reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
nCol <- 4
do.call("grid.arrange", c(interaction_plots[1:8], ncol=nCol))
do.call("grid.arrange", c(interaction_plots[9:16], ncol=nCol))

do.call("grid.arrange", c(interaction_plots[17:32], ncol=nCol))
do.call("grid.arrange", c(interaction_plots[33:48], ncol=nCol))
do.call("grid.arrange", c(interaction_plots[49:64], ncol=nCol))
```
...
test <- merge(test, abcd_psych %>% filter(eventname == 1) %>% select(-c("rel_family_id", "interview_age", "eventname")), by = c("src_subject_id"), all = TRUE)
...
```{r}
bw_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles_diff", "", rownames(bw_reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
```{r}
bwp_reg_table_full_sig_MC$label <- name_mapping$region_name[match(sub("_centiles_diff", "", rownames(bwp_reg_table_full_sig_MC)), name_mapping$abbreviation)]
...
```{r}
neo_comp_reg_table_full_sig$label <- name_mapping$region_name[match(sub("_centiles_diff", "", rownames(neo_comp_reg_table_full_sig)), name_mapping$abbreviation)]
...
ggplot(plot_data_dkt) +
  ...
  scale_fill_gradient2() +
  theme_void() +
  labs(title = "Cortical Regions with Neonatal Comp. pre-")
...
ggplot(plot_data_aseg) +
  ...
  scale_fill_gradient2() +
  theme_void() +
  labs(title = "Subcortical with Neonatal Comp. pre-")
```
###PRS-bw
```{r}
