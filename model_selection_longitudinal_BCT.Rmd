---
title: "model_selection_longitudinal_BCT"
author: "Eren Kafadar"
date: "2025-03-06"
...
out_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_models_long_BCT/" 

mu_terms = c("~1", "PCW_between_t1t2", "random(site)", "age", "sex", "nonSingleton", "age:sex")
...
split 1
sbatch array_script_submit.sh submit_gamlss_fits_ABCD.sh /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_models_long_BCT "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_models_long_BCT/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/process_tables_5.1/abcd5.1_t1t2_wide_sOne_train.csv" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_ONE/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_ONE/gamlss_fits_stats"

split 2
sbatch array_script_submit.sh submit_gamlss_fits_ABCD_TWO.sh /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_models_long_BCT "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_models_long_BCT/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/process_tables_5.1/abcd5.1_t1t2_wide_sTwo_train.csv" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_TWO/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_TWO/gamlss_fits_stats"
...
```{r}
all_fits_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/process_tables_5.1/longitudinal_all_fits_BCT.csv"
if(save_fits == TRUE){
...
```
##get lowest BIC model within each phenotype
```{r}
...
##IRRELEVANT, FIXED MODELS get table of which vars are chosen for which phenotype & which moment
```{r}
#terms
...
##Load best model
```{r}
sOne_list <- paste0(splitOne_folder,"gamlssFIT_",sOne_fits$model_index,".rds")
models_One <- lapply(sOne_list, readRDS)
sTwo_list <- paste0(splitTwo_folder,"gamlssFIT_",sTwo_fits$model_index,".rds")
models_Two <- lapply(sTwo_list, readRDS)
```

#Diagnostics
##Centile plots -- for figures
```{r}
model <- models_One[[112]]
...
ph.t1 <- sub(".t2", ".t1", phenotype)
    df <- sOne_df %>% 
      ...
      drop_na()
make_centile_fan(model, df, ph.t1 , desiredCentiles = c(0.05, 0.5, 0.95)) + labs(title = "Total Cortical WMV", x = "Timepoint 1", y = "Timepoint 2") +   
      geom_line(color = "#AF0F5A") +
      geom_point(color = "#AF0F5A", size = 1, alpha = 0.03) +
      theme(plot.title = element_text(hjust = 0.5, size = 20), axis.title.x = element_text(size = 16),  axis.title.y = element_text(size = 16),  panel.grid.major = element_blank(),panel.grid.minor = element_blank(), panel.background = element_blank())
```
##Worm plots
##Terms plots
...
if(diagnostics == TRUE){
plot <- FALSE
plots_term <- vector(mode = "list", length = dim(sOne_best)[1])
for (i  in 1:length(best_models_One)) {
  ...
  ph.t1 <- sub(".t2", ".t1", phenotype)
      df <- sOne_df %>% 
        select(sex, age, site, nonSingleton, PCW_between_t1t2, phenotype, ph.t1) %>% 
        drop_na()
        term.plot(model, what = "mu") + labs(title = title)
    }
...
for(k in 1:length(breaks)){
  ...
  do.call("grid.arrange", c(plots_term[start:end], ncol=nCol))
}
}
}
```
...
for (i  in 1:length(best_models_Two)) {
  ...
  ph.t1 <- sub(".t2", ".t1", phenotype)
      df <- sTwo_df %>% 
        select(sex, age, site, nonSingleton, PCW_between_t1t2, phenotype, ph.t1) %>% 
        drop_na()
      plots_term[[i]] <- term.plot(model, what = "mu") + labs(title = title)
    }
...
```
##Worm plots for top 5 BIC
get worm plots & centile plots for top 5 BIC, grouped by phenotype
###get top 5 BIC for each split and load those models
```{r}
sOne_best_five <- sOne_fits %>%
  filter(convergence_warn_end == 0) %>%
  group_by(phenotype) %>%
 slice_min(BIC, n = 5)

sTwo_best_five <- sTwo_fits %>%
  filter(convergence_warn_end == 0) %>%
  group_by(phenotype) %>%
  slice_min(BIC, n = 5)

sOne_best_five_list <- paste0(splitOne_folder,"gamlssFIT_",sOne_best_five$model_index,".rds")
fivebest_models_One <- lapply(sOne_best_five_list, readRDS)
sTwo_best_five_list <- paste0(splitTwo_folder,"gamlssFIT_",sTwo_best_five$model_index,".rds")
fivebest_models_Two <- lapply(sTwo_best_five_list, readRDS)
...
#plots_wp <- vector(mode = "list", length = 5)
#plots_cent <- vector(mode = "list", length = dim(sOne_best)[1])
for (i  in 1:length(fivebest_models_One)) {
  #print(deparse(model$mu.formula))
  model <- fivebest_models_One[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
      plots_wp[[i]] <- wp.taki(model, ylim = 1.5) + labs(title = title)
      df <- sOne_df %>% 
...
#plots_wp <- vector(mode = "list", length = 5)
#plots_cent <- vector(mode = "list", length = dim(stwo_best)[1])
for (i  in 1:length(fivebest_models_Two)) {
  #print(deparse(model$mu.formula))
  model <- fivebest_models_Two[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
      plots_wp[[i]] <- wp.taki(model, ylim = 1.5) + labs(title = title)
      df <- sTwo_df %>% 
...
```
##Worm plots by sex - for GMV
####splitOne
```{r}
phenotype <- "smri_vol_cdk_total.t2"
diagnostics = TRUE
if(diagnostics == TRUE){
plot <- FALSE
#plots_wp <- vector(mode = "list", length = dim(sTwo_best)[1])
plots_wp <- vector(mode = "list", length = 5)
#plots_cent <- vector(mode = "list", length = dim(stwo_best)[1])
for (i  in 1:length(fivebest_models_One[1:5])) {
  #print(deparse(model$mu.formula))
  model <- fivebest_models_One[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
  ph.t1 <- sub(".t2", ".t1", phenotype)
      df <- sOne_df %>% 
        select(sex, age, site, nonSingleton, PCW_between_t1t2, phenotype, ph.t1) %>% 
        drop_na()
      plots_wp[[i]] <- wp.taki(model, df$sex) + labs(title = title)
    }
...
plots_wp <- vector(mode = "list", length = 5)
#plots_cent <- vector(mode = "list", length = dim(stwo_best)[1])
for (i  in 1:length(fivebest_models_Two[1:5])) {
  #print(deparse(model$mu.formula))
  model <- fivebest_models_Two[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
...
for (i  in 1:length(fivebest_models_One[1:5])) {
  ...
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
        ...
        select(sex, age, site, nonSingleton, phenotype) %>% 
        drop_na()
      plots_wp[[i]] <- wp.taki(model, df$age) + labs(title = title)
     #plots_cent[[i]] <- make_centile_fan(model, df, "age", "sex", desiredCentiles = c(0.05, 0.5, 0.95)) + 
       #labs(title = titles[i],x = "Age", y = NULL, color = "Sex=Female", fill="Sex=Female")
    }
...
```
##Worm plots for ALL fitted models - GMV
Just to see if ANY of them look better ....
###Load all GMV models
```{r}
ph <- "smri_vol_cdk_total.t2"

models_One <- sOne_fits %>% filter(phenotype == ph & error == FALSE & convergence_warn_end == FALSE)
sOne_GMV_list <- paste0(splitOne_folder,"gamlssFIT_",models_One$model_index,".rds")
GMV_models_One <- lapply(sOne_GMV_list, readRDS)



models_Two <- sTwo_fits %>% filter(phenotype == ph & error == FALSE & convergence_warn_end == FALSE)
sTwo_GMV_list <- paste0(splitTwo_folder,"gamlssFIT_",models_Two$model_index,".rds")
GMV_models_Two <- lapply(sTwo_GMV_list, readRDS)
```
###sOne
```{r}
#diagnostics = TRUE
if(diagnostics == TRUE){
plot <- TRUE
plots_wp <- vector(mode = "list", length = dim(models_One)[1])
for (i  in 1:length(GMV_models_One)) {
  #print(deparse(model$mu.formula))
  model <- GMV_models_One[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
  ph.t1 <- sub(".t2", ".t1", phenotype)
      df <- sOne_df %>% 
...
if(diagnostics == TRUE){
plot <- TRUE
plots_wp <- vector(mode = "list", length = dim(models_Two)[1])
for (i  in 1:length(GMV_models_Two)) {
  #print(deparse(model$mu.formula))
  model <- GMV_models_Two[[i]]
  phenotype <- as.character(model$mu.terms[[2]])
  title <- name_mapping$region_name[match(sub(".t2", "",phenotype), name_mapping$abbreviation)]
  ph.t1 <- sub(".t2", ".t1", phenotype)
      df <- sTwo_df %>% 
...
model <- model_one 
out_train_one <- suppressMessages(cent_cdf(model_one, og_df_one))$data$fitted
#out_test_one <- suppressMessages(cent_cdf(model_one, sTwo_test_df))
centcdf_one$ph[i] <- title_one
centcdf_one$cdfcor_train[i] <- (cor.test(as.numeric(out_train_one), cdf_expected)$estimate)
...
#save correlation with expected cdf values
model <- model_two
out_train_two <- suppressMessages(cent_cdf(model_two, og_df_two))$data$fitted
#out_test_two <- suppressMessages(cent_cdf(model_two, sOne_test_df))
centcdf_two$ph[i] <- title_two
centcdf_two$cdfcor_train[i] <- (cor.test(as.numeric(out_train_two), cdf_expected)$estimate)
...
centiles_pred_one <- vector(mode = "list", length = dim(sOne_fits)[1])
centiles_pred_two <- vector(mode = "list", length = dim(sTwo_fits)[1])
splits_correlations <- setNames(data.frame(matrix(ncol = 4, nrow = dim(sOne_fits)[1])), c("ph_one", "cor_est_one", "ph_two", "cor_est_two"))

for(i in 1:length(models_One)){
#define models from split one and split two
model_one <- models_One[[i]]
model_two <- models_Two[sapply(models_Two, function(x){any(grepl(as.character(model_one$mu.formula[[2]]),x$mu.formula))})][[1]]
...
filename <- paste0(output_folder, "longmodel_all_volume_BCT_centiles.csv")
