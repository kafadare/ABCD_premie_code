...
centiles_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/Centile_CSVs/regional_baseline_volume_centiles.csv"
```
...
May not work out and have to do the old school way (might be more adaptable to adding different covariates)
```{r}
#regression (to be able to include covariates later)
# centiles_reg <- function(df, vars, mc_method = "BH") {
#   
# df <- df %>% mutate(across(names(df_vars),z_score))
# reg_table_full <- as.data.frame(t(sapply(df[, grepl("centiles", names(df))], function(x) {
#   
#   x_zscore <- z_score(x)
#   model_string <- paste0("x_zscore", paste(vars[1:length(vars)], collapse = "+"))
#   formula <-as.formula(model_string)
#   lm <- lm(formula)
#   summary <- summary(lm)
#   for(i in 1:length(vars)){
#     colnames_beta <- paste0(names(vars),"_beta")
#     colnames_p <- paste0(names(vars),"_p")
#     ## ?? How to adapt this part to dynamic column # possibility???
...
# reg_table_full$ga_p_adj <- p.adjust(reg_table_full$ga_p, method = mc_method)
# reg_table_full_sig_MC <- reg_table_full %>% filter(ga_p_adj < 0.05)
# return(reg_table_full, reg_table_full_sig, reg_table_full_sig_MC)
# }
```
#Additional Data
##Merge data
...
```{r}
# ##save files for birthweight available in pounds
#male_bw_lbs <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% filter(sex == "M")
#female_bw_lbs <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% filter(sex == "F")
# 
#write.csv(male_bw_lbs, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_lbs.csv")
#write.csv(female_bw_lbs, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/female_birthweight_lbs.csv")

# ##save files for birthweight available in pounds + 0.5 pounds
# male_bw_lbsadj <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% 
#   filter(sex == "M") %>%
#   mutate(birth_weight_lbs_adj = birth_weight_lbs + 0.5) %>%
#   mutate(birth_weight_grams_adj = birth_weight_lbs_adj * 453.592) %>%
#   select(src_subject_id, sex, birth_weight_lbs, birth_weight_lbs_adj, gestAge, birth_weight_grams_adj)
# 
# female_bw_lbsadj <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_lbs")) %>% 
#   filter(sex == "F") %>%
#   mutate(birth_weight_lbs_adj = birth_weight_lbs + 0.5) %>%
#   mutate(birth_weight_grams_adj = birth_weight_lbs_adj * 453.592) %>%
#   select(src_subject_id, sex, birth_weight_lbs, birth_weight_lbs_adj, gestAge, birth_weight_grams_adj)
# 
# write.csv(male_bw_lbsadj, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_lbsadj.csv")
# write.csv(female_bw_lbsadj, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/female_birthweight_lbsadj.csv")
# 
# ##save files for birthweight available in ounces
# male_bw_oz <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_oz")) %>% 
#   filter(sex == "M") %>%
#   mutate(birth_weight_grams = birth_weight_oz * 28.350) %>%
#   select(src_subject_id, sex, birth_weight_oz, gestAge, birth_weight_grams)
# female_bw_oz <- test_prs %>% select(c("src_subject_id", "sex", "gestAge", "birth_weight_oz")) %>% 
#   filter(sex == "F") %>%
#   mutate(birth_weight_grams = birth_weight_oz * 28.350) %>%
#   select(src_subject_id, sex, birth_weight_oz, gestAge, birth_weight_grams)
# 
# write.csv(male_bw_oz, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_oz.csv")
# write.csv(female_bw_oz, "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/female_birthweight_oz.csv")
...
all_birthweight <- rbind(read.csv(filename_bwcentile_boy, na.strings=c("","NA")), read.csv(filename_bwcentile_girl, na.strings=c("","NA"))) %>% select(-1) %>%
  rename(bweight_percentile_old = bweight_percentile)

#all_birthweight$bw_category <- ifelse(is.na(all_birthweight$birth_weight_grams), NA, all_birthweight$bw_category)
test_prs <- merge(test_prs, all_birthweight %>% select(c("src_subject_id", "birth_weight_grams", "bweight_percentile_old")), by = c("src_subject_id"), all.x = TRUE)

```
rsync -avz /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/male_birthweight_lbsadj.csv /home/kafadare/tsclient/ekafadar/Documents/Grad_School/BGDLab/ABCD_data/
...
#check correlation with centiles calculated from Fenton charts
#plot(test_prs$bweight_percentile, test_prs$bweight_percentile_old)
#cor.test(test_prs$bweight_percentile, test_prs$bweight_percentile_old)
```
...
##Correlations with non-centiled raw (z-scored) imaging variables
###Gestational Age
```{r}
ga_zscore <- z_score(test_prs$gestAge)
age_zscore <- z_score(test_prs$age)
reg_table_full <- as.data.frame(t(sapply(test_prs[, sub("_centiles", "", names(test_prs)[grepl("centiles", names(test_prs))])], function(x) {
  x_zscore <- z_score(x)
  lm <- lm(x_zscore ~ ga_zscore + age_zscore + test_prs$sex + test_prs$nonSingleton)
  summary <- summary(lm)
...
bw_zscore <- z_score(test_prs$birth_weight_sum_oz)
age_zscore <- z_score(test_prs$age)
bw_reg_table_full <- as.data.frame(t(sapply(test_prs[, sub("_centiles", "", names(test_prs)[grepl("centiles", names(test_prs))])], function(x) {
  x_zscore <- z_score(x)
  lm <- lm(x_zscore ~ bw_zscore + age_zscore + test_prs$sex + test_prs$nonSingleton)
  summary <- summary(lm)
...
reg_table_full <- as.data.frame(t(sapply(test_prs[, sub("_centiles", "", names(test_prs)[grepl("centiles", names(test_prs))])], function(x) {
  x_zscore <- z_score(x)
  lm <- lm(x_zscore ~ ga_zscore*bw_zscore + age_zscore + test_prs$sex + test_prs$nonSingleton)
  summary <- summary(lm)
  
  c(ga_beta = summary$coefficients[2,1], ga_p = summary$coefficients[2,4], bw_beta = summary$coefficients[3,1], bw_p = summary$coefficients[3,4], gabw_beta = summary$coefficients[7,1], gabw_p = summary$coefficients[7,4])
})))
...
```
##Plot Beta Ranges
###Gestational Age - centile scores
```{r}
ga_zscore <- z_score(test_prs$gestAge)
reg_table_full <- as.data.frame(t(sapply(test_prs[, grepl("centiles", names(test_prs))], function(x) {
  x_zscore <- qnorm(x)
...
ggplot(reg_table_full_sig_MC, aes(y = label, x = ga_beta, xmin = ga_lower, xmax = ga_upper)) +
  geom_pointrange() +
  theme_minimal() +
