...
wide_combined_reg_table$survive_tbv <- (sign(wide_combined_reg_table$ga_beta_uni) == sign(wide_combined_reg_table$ga_beta_tbv)) & wide_combined_reg_table$ga_p_adj_uni < 0.5 & wide_combined_reg_table$ga_p_adj_tbv < 0.5

#visualize the global phenotypes betas
ggplot() + geom_pointrange(data = reg_table_full_sig_MC %>% filter(rownames %in% global_phenotypes), aes(y = label, x = ga_beta, xmin = ga_lower, xmax = ga_upper, color = blue)) +
