...
source("/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/ABCD_premie_code/wp_taki_EK.R")
...
out_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/"
...
#generate = TRUE
if (generate == TRUE){
  models_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/"
...
split 1
sbatch array_script_submit.sh submit_gamlss_fits_ABCD_ONE.sh /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0 "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/process_tables_5.1_2.0/abcd_long_Onetrain2025-06-16.csv" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t1_BCT_vol_splitONE_2.0/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t1_BCT_vol_splitONE_2.0/gamlss_fits_stats" "t1"

split 2
sbatch array_script_submit.sh submit_gamlss_fits_ABCD_TWO.sh /mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0 "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/process_tables_5.1_2.0/abcd_long_Twotrain2025-06-16.csv" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t1_BCT_vol_splitTWO_2.0/" "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_fits_t1_BCT_vol_splitTWO_2.0/gamlss_fits_stats" "t1"
#SETUP Data
##Define files
```{r}
#file paths
...
```{r}
#save_fits = TRUE
all_fits_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/t1_BCT_fits.csv"
if(save_fits == TRUE){
...
##Inspect Variables in Best Models
...
##sOne
Total_Cerebellum-WM
Right_Cerebelum-White-Matter
Left_Cerebellum-White-Matter
...
After BCT
Left-Lateral-Ventricle (18%)
```{r}
# sOne_refit_phenotypes <- c("Total_Cerebellum-WM"
...
#                            ,"rh_precentral"
#                            ,"lh_entorhinal")
sOne_refit_phenotypes <- c("Left-Lateral-Ventricle")

sOne_best %>% filter(label %in% sOne_refit_phenotypes) %>% select(c(mu_formula))
sOne_best %>% filter(label %in% sOne_refit_phenotypes) %>% select(c(sigma_formula))
sOne_best %>% filter(label %in% sOne_refit_phenotypes) %>% select(c(nu_formula))
...
```
### test refit() and update() the models
```{r}
...
model_specs_list <- list.files("/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/")
refit_model_specs_list <- model_specs_list[indexes$model_index]
refit_model_specs <- lapply(paste0("/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/2.0_results/gamlss_models_t1_BCT_vol_2.0/",refit_model_specs_list), readRDS)
...
title <- name_mapping$region_name[match(as.character(phenotype), name_mapping$abbreviation)]
df <- sOne_train_df %>% select(sex, age, site, nonSingleton, phenotypes_to_fit) %>% 
    drop_na()

new_form <- as.formula(paste0(deparse(refit_models_One[[1]]$nu.formula), "+ age"))

model_new <- update(model_old, what = "nu", formula. = new_form, data = df)

wp.taki(model_new, ylim.worm = 0.5, xlim.worm = 4) + labs(title = paste0(title))

model_new <-  gamlss (formula = model$mu.formula,
                  ...
                  start.from = model_old)


wp.taki(model_new, ylim.worm = 0.5, xlim.worm = 4) + labs(title = paste0(title))

gamlss_gg <- gamlss (formula = model$mu.formula,
                  sigma.formula = model$sigma.formula,
                  nu.formula = model$nu.formula,
                  family = "GG",
                  method = RS(),
...
gamlss_bct <- gamlss (formula = model$mu.formula,
                  ...
                  data = df,
                  control = gamlss.control(n.cyc = eval(model$n_crit)),
                  start.from = gamlss_gg)

wp.taki(gamlss_gg, ylim.worm = 0.5, xlim.worm = 4) + labs(title = paste0(title, ": GG"))
wp.taki(gamlss_bct, ylim.worm = 0.5, xlim.worm = 4) + labs(title = paste0(title, ": BCT"))
}
...
CC_Central
```{r}
# sTwo_refit_phenotypes <- c("Total_Cerebellum-WM"
#                            ,"Right-Cerebellum-White-Matter"
#                            ,"Left-Cerebellum-White-Matter"
#                            ,"Right-Inf-Lat-Vent"
#                            ,"Left-Inf-Lat-Vent"
...
#                            ,"rh_precentral"
#                            ,"lh_entorhinal")
sTwo_refit_phenotypes <- c("Left-Lateral-Ventricle",
                           "Right-Lateral-Ventricle",
                           "CC_Central")

sTwo_best %>% filter(label %in% sTwo_refit_phenotypes) %>% select(c(mu_formula))
sTwo_best %>% filter(label %in% sTwo_refit_phenotypes) %>% select(c(sigma_formula))
sTwo_best %>% filter(label %in% sTwo_refit_phenotypes) %>% select(c(nu_formula))

sTwo_refit_indexes <- sTwo_fits %>% filter(label %in% sTwo_refit_phenotypes) %>% select(model_index)

sTwo_refit_list <- paste0(splitTwo_folder,"gamlssFIT_",sTwo_refit_indexes$model_index,".rds")
refit_models_Two <- lapply(sTwo_refit_list, readRDS)
...
sTwo_refit_list <- paste0(splitTwo_folder,"gamlssFIT_",indexes$model_index,".rds")
refit_models_Two <- lapply(sTwo_refit_list, readRDS)
...
new_form <- as.formula(paste0(deparse(refit_models_Two[[1]]$nu.formula), "+ age"))
...
```{r}
# assume models is a named list of gamlss objects
extract_model_info <- function(mod, phenotype) {
  fam <- family(mod)[[1]]
  bic <- BIC(mod)
  
  coefs <- map(c("mu", "sigma", "nu", "tau"), function(par) {
    if (par %in% names(mod$parameters)) {
      cf <- coef(mod, what = par)
      if (length(cf) > 0) {
        tibble(term = names(cf), value = unname(cf), moment = par)
      } else {
        tibble(term = character(0), value = numeric(0), moment = character(0))
      }
    } else {
      tibble(term = character(0), value = numeric(0), moment = character(0))
    }
  ...
  coefs %>%
    mutate(term = paste(moment, term, sep = "_")) %>%
    select(-moment) %>%
    pivot_wider(names_from = term, values_from = value) %>%
    mutate(phenotype = phenotype, BIC = bic, family = fam, .before = 1)
}

all_models_df <- imap_dfr(models, ~ extract_model_info(.x, .y))
```