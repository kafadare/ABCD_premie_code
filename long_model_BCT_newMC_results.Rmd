---
title: "long_model_BCT_newMC_results"
author: "Eren Kafadar"
date: "2025-04-11"
...
```{r}
#file paths
splitOne_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_ONE/"
splitTwo_folder <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/gamlss_fits_long_BCT_TWO/"
...
centiles_filename <- "/mnt/isilon/bgdlab_processing/Eren/ABCD-braincharts/CSV/Centile_CSVs/longmodel_all_volume_BCT_centiles.csv"
```
...
test <- merge(test, abcd_psych %>% filter(eventname == 1) %>% rename(General_p_baseline = General_p) %>% select(c("General_p_baseline", "src_subject_id")), by = c("src_subject_id"), all.x = TRUE)
...
test_prs$General_p_change <- test_prs$General_p - test_prs$General_p_baseline
```
...
cor.test(test_prs$gestAge, test_prs$centile_md, method = "spearman")
...
if(save_tables == TRUE){
  ...
  filename_all <- paste0(output_folder, "dsk_gestAge_long_all.csv")
  filename_sig <- paste0(output_folder, "dsk_gestAge_long_sig.csv")
...
if(save_tables == TRUE){
  ...
  save_table_sig <- save_table_full
  save_table_sig[which(save_table_sig$bw_p_adj>0.05), "bw_beta"] <- 0
  filename_all <- paste0(output_folder, "dsk_birthweight_percentile_long_all.csv")
  filename_sig <- paste0(output_folder, "dsk_birthweight_percentile_long_sig.csv")
  write.csv(save_table_full, filename_all)
  write.csv(save_table_sig, filename_sig)
}
```
...
if(save_tables == TRUE){
  ...
  filename_all <- paste0(output_folder, "dsk_birthweightPGS_long_all.csv")
  filename_sig <- paste0(output_folder, "dsk_birthweightPGS_long_sig.csv")
  write.csv(save_table_full, filename_all)
  write.csv(save_table_sig, filename_sig)
}
```
