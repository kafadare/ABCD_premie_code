...
bw_pgs_betas_left <- bw_betas %>% filter(hemisphere == "L")
bw_pgs_betas_right <- bw_betas %>% filter(hemisphere == "R")
...
bw_pgs_betas_left <- merge(bw_betas_left, left_labels[,c("label", "Key")], by = "label")
bw_pgs_betas_right <- merge(bw_betas_right, right_labels[,c("label", "Key")], by = "label")
